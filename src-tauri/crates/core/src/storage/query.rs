...
impl Q {
    ...
    pub fn new(sql: impl Into<String>, param: impl ToSql + 'static) -> Self {
        Q { sql: sql.into(), params: vec![Box::new(param)] }
    }

    /// A param-free fragment, e.g. `Q::raw("PROJECT_FK IS NULL")`.
    pub fn raw(sql: impl Into<String>) -> Self {
        Q { sql: sql.into(), params: Vec::new() }
    }
    ...
    pub fn not(self) -> Q {
        Q { sql: format!("(NOT {})", self.sql), params: self.params }
    }
...
pub fn _in<T: ToSql + 'static>(col: &str, vals: impl IntoIterator<Item = T>) -> Q {
    let params: Vec<Box<dyn ToSql>> = vals.into_iter().map(|v| Box::new(v) as Box<dyn ToSql>).collect();
    let marks = vec!["?"; params.len()].join(",");
    Q { sql: format!("{col} IN ({marks})"), params }
}
...
mod tests {
    ...
    fn compose_and_or_not_and_in() {
        let q = Q::new("a = ?", 1i64).and(Q::new("b = ?", 2i64)).or(Q::new("c = ?", 3i64).not());
