...
//!
//! Pure parser over PDF bytes: pdf-extract for first-page text, its lopdf
//! re-export for the Info dict (/Title /Author /CreationDate). The arXiv-id and
...
// ---------------------------------------------------------------------------
// Info dict (lopdf, re-exported by pdf-extract)
// ---------------------------------------------------------------------------
...
/// First-page plain text via pdf-extract's single-page renderer (1-based).
fn first_page_text(doc: &pdf_extract::Document) -> String {
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let mut s = String::new();
        {
            let mut out = pdf_extract::PlainTextOutput::new(&mut s);
            let _ = pdf_extract::output_doc_page(doc, &mut out, 1);
        }
        s
    }))
    .unwrap_or_default()
...
fn info_dict(doc: &pdf_extract::Document) -> Option<&pdf_extract::Dictionary> {
    match doc.trailer.get(b"Info").ok()? {
        pdf_extract::Object::Reference(id) => match doc.get_object(*id).ok()? {
            pdf_extract::Object::Dictionary(d) => Some(d),
            _ => None,
        },
        pdf_extract::Object::Dictionary(d) => Some(d),
        _ => None,
...
fn info_str(info: &pdf_extract::Dictionary, key: &[u8]) -> Option<String> {
...
pub(crate) fn extract_pdf_metadata(bytes: &[u8]) -> Extracted {
    ...
    let doc = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        pdf_extract::Document::load_mem(bytes).ok()
    }))
    .ok()
    .flatten();
    ...
    // First-page text ONLY — Python reads pages[0]. Extracting every page (e.g.
    // extract_text_from_mem_by_pages) made a 30-page paper take ~7min; page 1 is
    // sub-second. Reuse the already-loaded doc. catch_unwind: the content-stream
    // parser can panic on some PDFs — degrade, don't crash.
...
mod tests {
    ...
    // ---- D10 SPIKE: real committed PDFs (pdf-extract, no network) ----
    ...
    // SPIKE FINDING: pdf-extract renders this one paper's FIRST page in ~3 min
    // (debug) — a content-stream pathology, not the all-pages cost. Extraction
    // is *correct*, just pathologically slow on this file. Ignored so the suite
    // stays fast; run `cargo test -- --ignored` to confirm. Signals the pdfium
    // fallback (or a render timeout/spawn_blocking deadline) for production.
    #[test]
    #[ignore = "pdf-extract ~3min on this PDF's first page; run with --ignored"]
    fn spike_multi_author_2411_slow() {
        let m = extract_pdf_metadata(&read_test_pdf("2411.10406v2.pdf"));
        // REQUIRED spike asserts: arxiv id == filename id, first-page text present.
        assert_eq!(m.arxiv_id.as_deref(), Some("2411.10406v2"));
        let title = m.title.expect("first-page text title extracted");
        assert!(!title.is_empty());
        assert!(
            title.contains("Quantum Supercomputer"),
            "got title: {title:?}"
        );
        assert_eq!(m.authors, None, "embedded /Author is empty");
        assert_eq!(m.doi, None, "no DOI in first-page text");
    }
